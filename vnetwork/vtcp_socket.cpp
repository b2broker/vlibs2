...
{
    if (!addr.is_valid())
        throw verror << "Not valid socket address for connect.";
    ...
    if (conn_ok)
        throw verror << "Bad connect logic!";
...
//---------------------------------------------------------------------------------------
vtcp_socket::_pimpl::_pimpl(vtcp_socket *owner_) : owner(owner_), stage(Unconnected) {}
//---------------------------------------------------------------------------------------
vtcp_socket::_pimpl::_pimpl(vtcp_socket *owner_, accepted_peer *peer)
    : owner(owner_), stage(Connected)
{
    fd = peer->_take_fd();
...
{
    if (owner->dbg_log)
        vdeb << "vtcp_socket::on_events";

    if (e.take_error())
    {
        if (owner->dbg_log)
            vdeb << "vtcp_socket::on_events error";
    ...
    {
        if (owner->dbg_log)
            vdeb << "vtcp_socket::on_events read";
        when_ready_read();
    }
    ...
    {
        if (owner->dbg_log)
            vdeb << "vtcp_socket::on_events write";
        when_ready_write();
    }
    ...
    {
        if (owner->dbg_log)
            vdeb << "vtcp_socket::on_events take_read_hang_up";
    ...
    if (owner->dbg_log)
    {
        vdeb << "vtcp_socket::on_events check";
        e.check_empty();
    }
}
//---------------------------------------------------------------------------------------
...
void vtcp_socket::_pimpl::when_ready_read()
{
    ...
    //    vdeb << "when_ready_read" << res.empty();

    if (!res.empty())
    {
        if (owner->dbg_log)
            vdeb << "send to owner" << res.size();
...
//=======================================================================================
vtcp_socket::vtcp_socket() : _p(new _pimpl(this)) {}
//---------------------------------------------------------------------------------------
vtcp_socket::vtcp_socket(accepted_peer *peer) : _p(new _pimpl(this, std::move(peer))) {}
//---------------------------------------------------------------------------------------
...
void vtcp_socket::send(const std::string &data)
{
    ...
    auto res = _p->send(data);
    vdeb << "!vtcp_socket::send " << res << "size: " << data.size();
    if (!res)
    {
...
vtcp_socket::accepted_peer::accepted_peer(int fd, vsocket_address &&peer_addr)
    : _fd(std::make_shared<int>(fd)), _peer(std::move(peer_addr))
{}
//---------------------------------------------------------------------------------------
